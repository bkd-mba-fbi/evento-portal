define([
    ...
    app.FixedHeaderTableComponent = ember.Component.extend({
        ...
        createSortControls: function() {
            var that = this;
            var table = this.$();
            table.addClass('sorting-header');
            table.find('th[is-sortable]').each(function(index, th) {
                var headerCell = $(th);
                ...
                headerText.click(function () {
                        ...
                        if (i !== currentSortedIndex) {
                            table.find('.' + guiHelpers.classes.table.sortingDirection)
