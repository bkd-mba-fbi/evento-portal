define([
    ...
    var api = {
        ...
        createSubscription: function(idPerson, idEvent, success) {
            var subscription = {
                IdPerson: idPerson,
                IdEvent: idEvent
            };
            api.post('Subscriptions/', subscription, success);
        },

        updateSubscriptionDetail: function(subscriptionDetail, success) {
            api.put('SubscriptionDetails/' + subscriptionDetail.IdObject, subscriptionDetail, success);
        },
        ...
        getAvailableCrystalReports: function(context, ids, keys, success) {
            var idString = ids;
            ...
            api.get('CrystalReports/AvailableReports/' + context + '?ids=' + idString + '&keys=' + keyString, success);
        },
        ...
        //Grading
        updateGradingItem: function(model, success) {
            api.put('GradingItems/' + model.IdSubscription, model, success);
        },

        updateGradingItems: function(idEvent, models, success) {
            api.put('GradingItems/?idEvent=' + idEvent, models, success);
        },

        updateExcelGradingList: function(file, idEvent, ads, success) {
        ...
        //PersonalDataSheet
        createPersonalDataSheet: function (success) {
            api.post('PersonalDataSheets/', { IdPerson: storage.userSettings.personId() }, success);
        },

        updatePersonalDataSheet: function (model, success) {
            api.put('PersonalDataSheets/' + model.Id, model, success);
        },
        
        //PersonalDataAnswer
        createPersonalDataAnswer: function (idDataSheet, idQuestion, yesNo, cantonIdELK, nationalityIdLFS, personalDataAnswerParagraphD, success) {
            var answer = {
            ...
            api.post('PersonalDataAnswers/', answer, success);
        },

        updatePersonalDataAnswer: function (model, success) {
            api.put('PersonalDataAnswers/' + model.Id, model, success);
        },

        deletePersonalDataAnswer: function (idPda, success) {
            api.del('PersonalDataAnswers/' + idPda, success);
        },

        //Status
        processStatus: function(statusProcess, success) {
            api.post('StatusProcesses/', statusProcess, success);
        },
                ...
                api.get(relativeUrl,
                    ...
                    function(jqXHR, textStatus, errorThrown) {
                        if (jqXHR.status === 409) {
                            resolve({
                                hasError: true,
                                errorMessage: JSON.parse(jqXHR.responseJSON)
                            });
                            return false;
                        }
        ...
        handleError: function (method, url, error, success, jqXHR, textStatus, errorThrown) {
            ...
            if (jqXHR.status === 409) {
                ...
                storage.errorInfo({
                    ...
                    message: jqXHR.responseJSON !== undefined ? JSON.parse(jqXHR.responseJSON) : jqXHR.responseText,
                    isGet: method === 'GET',
                    isPost: method === 'POST',
                    isPut: method === 'PUT',
