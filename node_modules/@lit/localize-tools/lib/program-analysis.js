 ...
 */
export function extractMessagesFromProgram(program) {
    const messages = [];
    const errors = [];
...
function extractMsg(file, program, node) {
    ...
    if (node.arguments.length < 1 || node.arguments.length > 2) {
        return {
            error: createDiagnostic(file, node, `Expected 1 or 2 arguments to msg(), got ${node.arguments.length}`),
        };
    }
 ...
 */
export function extractOptions(node, file) {
    if (node === undefined) {
        return { result: {} };
    }
    if (!ts.isObjectLiteralExpression(node)) {
        return {
            error: createDiagnostic(file, node, `Expected second argument to msg() to be an object literal`),
        };
    }
    let id;
    let desc;
    for (const property of node.properties) {
        ...
        if (!ts.isPropertyAssignment(property)) {
            return {
                error: createDiagnostic(file, property, `Options object must use identifier or string literal property ` +
        ...
        if (ts.isIdentifier(property.name)) {
            ...
            return {
                error: createDiagnostic(file, property.name, `Options object must use identifier or string literal property ` +
 ...
 */
export function extractTemplate(templateArg, file, typeChecker) {
    ...
    if (ts.isTemplateExpression(templateArg) ||
        ...
        return {
            error: createDiagnostic(file, templateArg, `String literal with expressions must use the str tag`),
        };
    }
    return {
...
export function generateMsgIdFromAstNode(template, isHtmlTagged) {
    const strings = [];
...
const EXPRESSION_START_END_REGEXP = new RegExp(EXPRESSION_START + '(.*?)' + EXPRESSION_END, 'g');
/**
 ...
 */
function replaceExpressionsAndHtmlWithPlaceholders(parts) {
    const concatenatedHtml = parts
        .map((part, expressionIndex) => typeof part === 'string'
        ? part
        : EXPRESSION_START +
        ...
        if (typeof part === 'string') {
            ...
            for (const substr of startSplit) {
                ...
                if (endSplit.length === 1) {
                    ...
                    const expression = parts[expressionIndex].identifier;
                    contents.push({
            ...
            contents.push({
                untranslatable: part.untranslatable.replace(EXPRESSION_START_END_REGEXP, (_, expressionIndexStr) => {
                    // Swap the expression index with the actual expression.
                    const expressionIndex = Number(expressionIndexStr);
                    const expression = parts[expressionIndex]
                        .identifier;
                    return '${' + expression + '}';
                }),
            });
 ...
 *   [ {ph: '<b>${foo}</b>'} ]
 */
function combineAdjacentPlaceholders(original) {
    const combined = [];
    const phBuffer = [];
    let phIdx = 0;
...
function replaceHtmlWithPlaceholders(html) {
    ...
    const traverse = (node) => {
        if (node.nodeName === '#text') {
            ...
            components.push({
                untranslatable: serializeComment(node),
            });
        }
        else {
 ...
 */
export function isLitTemplate(node) {
 ...
 * E.g. str`foo${bar}`
 */
export function isStrTemplate(node) {
    return (ts.isTaggedTemplateExpression(node) &&
        ts.isIdentifier(node.tag) &&
 ...
 */
export function isMsgCall(node, typeChecker) {
 ...
 */
export function isStrTaggedTemplate(node, typeChecker) {
...
//# sourceMappingURL=program-analysis.js.map