 ...
 */
export function readConfigFileAndWriteSchema(configPath) {
    let str;
    try {
    ...
    const schemaPath = pathLib.resolve(dirname(fileURLToPath(import.meta.url)), '..', 'config.schema.json');
    const schema = JSON.parse(fs.readFileSync(schemaPath, 'utf8'));
    const result = jsonSchema.validate(parsed, schema);
    if (result.errors.length > 0) {
        throw new KnownError(`Error validating config file ${configPath}:\n\n` +
    ...
    const config = {
        ...validated,
        baseDir,
        resolve: (path) => pathLib.resolve(baseDir, path),
    };
    return config;
}
/**
 ...
 */
function writeConfigSchemaIfMissing(config, configPath) {
    if ('$schema' in config) {
        return;
    }
    const withSchema = {
