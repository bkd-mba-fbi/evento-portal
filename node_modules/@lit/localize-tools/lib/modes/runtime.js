...
import { escapeTextContentToEmbedInTemplateLiteral, parseStringAsTemplateLiteral, } from '../typescript.js';
...
export class RuntimeLitLocalizer extends LitLocalizer {
    constructor(config) {
        ...
        if (config.output.mode !== 'runtime') {
            throw new Error(`Error: TransformLocalizer requires a localization config with output.mode "runtime"`);
        }
        this.config = config;
    }
    async build() {
 ...
 */
async function runtimeOutput(messages, translationMap, config, runtimeConfig) {
    let fileExtension;
    ...
    if (runtimeConfig.localeCodesModule) {
        writes.push(writeLocaleCodesModule(config.sourceLocale, config.targetLocales, runtimeConfig.localeCodesModule));
    }
    ...
    try {
        ...
        throw new KnownError(`Error creating locales directory: ${outputDir}\n` +
            `Do you have write permission?\n` +
            e.message);
    }
    for (const locale of config.targetLocales) {
        const translations = translationMap.get(locale) || [];
        const ts = generateLocaleModule(locale, translations, messages, config.patches || {});
        const filename = pathLib.join(outputDir, `${locale}.${fileExtension}`);
        writes.push(fsExtra.writeFile(filename, ts, 'utf8').catch((e) => {
            throw new KnownError(`Error writing file: ${filename}\n` +
                `Do you have write permission?\n` +
                e.message);
        }));
    }
 ...
 */
function generateLocaleModule(locale, translations, canonMsgs, patches) {
    ...
    for (const msg of translations) {
        ...
        if (canon === undefined) {
            console.warn(`${locale} message ${msg.name} does not exist in canonical messages, skipping`);
            continue;
        }
    ...
    for (const msg of canonMsgs) {
        ...
        console.warn(`${locale} message ${msg.name} is missing, using canonical text as fallback`);
 ...
 */
function copyMessagesSortedByName(messages) {
