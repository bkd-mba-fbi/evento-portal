 ...
 */
export function getOneElementByTagNameOrThrow(parent, tagName) {
    const matches = parent.getElementsByTagName(tagName);
    if (matches.length !== 1) {
        const parentName = 'tagName' in parent ? `<${parent.tagName}>` : 'document';
        throw new Error(`Expected 1 <${tagName}> in ${parentName}, got ${matches.length}`);
    }
    return matches[0];
}
/**
 ...
 */
export function getNonEmptyAttributeOrThrow(element, attributeName) {
    const attribute = element.getAttribute(attributeName);
    if (!attribute) {
        throw new Error(`Expected <${element.tagName}> to have ` +
